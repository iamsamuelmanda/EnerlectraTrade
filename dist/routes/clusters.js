...
const clusterService = new clusterService_1.ClusterService();
...
router.get('/', async (req, res) => {
    try {
        ...
        // Apply filters
        if (region) {
            clusters = clusters.filter(c => c.location.region.toLowerCase().includes(region.toLowerCase()));
        }
        if (type) {
...
router.get('/:clusterId', async (req, res) => {
    try {
        ...
        const response = {
            success: false,
            message: `Failed to fetch cluster: ${error.message}`
        };
        res.status(500).json(response);
    }
});
    ...
    try {
        ...
        const dashboard = {
            ...
            recentActivity: {
                ...
                newMembers: cluster.members.filter(m => new Date(m.joinedAt) > new Date(Date.now() - 7 * 24 * 60 * 60 * 1000)).length,
                activeProposals: 2, // Would fetch from decisions
                maintenanceDue: cluster.sharedAssets.filter(asset => asset.maintenanceSchedule.some(m => new Date(m.scheduledDate) < new Date(Date.now() + 7 * 24 * 60 * 60 * 1000))).length
            }
        };
        const response = {
            success: true,
...
router.post('/:clusterId/equipment/propose', async (req, res) => {
    try {
        ...
        const response = {
            success: false,
            message: `Failed to propose equipment purchase: ${error.message}`
        };
        res.status(400).json(response);
    }
});
...
router.post('/:clusterId/energy/distribute', async (req, res) => {
    try {
        ...
        const response = {
            success: false,
            message: `Failed to distribute energy: ${error.message}`
        };
        res.status(500).json(response);
    }
});
...
router.get('/:clusterId/returns', async (req, res) => {
    try {
        ...
        const response = {
            success: false,
            message: `Failed to calculate returns: ${error.message}`
        };
        res.status(500).json(response);
    }
});
...
router.get('/user/:userId', async (req, res) => {
    try {
        ...
        const response = {
            success: false,
            message: `Failed to fetch user clusters: ${error.message}`
        };
        res.status(500).json(response);
    }
});
...
router.get('/region/:region', async (req, res) => {
    try {
        ...
        const response = {
            success: false,
            message: `Failed to fetch regional clusters: ${error.message}`
        };
        res.status(500).json(response);
    }
});
