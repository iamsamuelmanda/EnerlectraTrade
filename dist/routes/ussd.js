                ...
                if (menuLevels[0] === '2') {
                    ...
                    if (clusterIndex >= 0 && clusterIndex < clusters.length && kWhToBuy > 0) {
                        ...
                        if (user.balanceZMW >= totalCost && selectedCluster.availableKWh >= kWhToBuy) {
                            // Execute purchase
                            (0, utils_1.updateUserBalance)(users, user.id, -totalCost, kWhToBuy);
                            // Update cluster
                            clusters[clusterIndex].availableKWh -= kWhToBuy;
                            // Create transaction
                            const transactions = (0, utils_1.readJsonFile)('transactions.json');
                            const transaction = (0, utils_1.createTransaction)('purchase', {
                                userId: user.id,
