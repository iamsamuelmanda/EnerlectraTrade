...
import { Cluster, Transaction, ApiResponse } from '../types';
  ...
  try {
    ...
    const clusterStats = clusters.map(cluster => {
      ...
      const recentActivity = transactions.filter(t => 
        t.clusterId === cluster.id && new Date(t.timestamp) > oneHourAgo
      );

      const dailyActivity = transactions.filter(t => 
        t.clusterId === cluster.id && new Date(t.timestamp) > oneDayAgo
      );
    ...
    const response: ApiResponse = {
      ...
      data: {
        ...
        recommendations: {
          immediate: clusterStats.filter(c => c.status.health === 'critical').map(c => 
            `${c.location}: ${c.status.alerts.join(', ')}`
          ),
          optimization: [
...
router.get('/cluster/:clusterId', (req: Request, res: Response) => {
  try {
    ...
    const clusterTransactions = transactions.filter(t => 
      t.clusterId === clusterId && new Date(t.timestamp) > timeThreshold
    );
    ...
    const hourlyStats = Object.values(hourlyData).map((data: any) => ({
