...
import { readJsonFile, writeJsonFile, generateId, updateUserBalance, createTransaction } from '../utils/common';
import { User, Cluster, ApiResponse } from '../types';
...
router.post('/trade', (req: Request, res: Response) => {
  try {
    ...
    const scheduledTransactions = readJsonFile<ScheduledTransaction>('scheduled_transactions.json');

    const scheduledTrade: ScheduledTransaction = {
      id: generateId(),
      type: 'trade',
      scheduledTime,
      status: 'pending',
    ...
    writeJsonFile('scheduled_transactions.json', scheduledTransactions);
...
router.post('/purchase', (req: Request, res: Response) => {
  try {
    ...
    const scheduledPurchase: ScheduledTransaction = {
      id: generateId(),
      type: 'purchase',
      scheduledTime,
      status: 'pending',
...
router.get('/:userId', (req: Request, res: Response) => {
  try {
    ...
    let scheduledTransactions = readJsonFile<ScheduledTransaction>('scheduled_transactions.json');
    
    // Filter by user
    scheduledTransactions = scheduledTransactions.filter(st => 
      st.buyerId === userId || st.sellerId === userId || st.userId === userId
    );
...
router.post('/execute', (req: Request, res: Response) => {
  try {
    ...
    const pendingTransactions = scheduledTransactions.filter(st => 
      st.status === 'pending' && new Date(st.scheduledTime) <= now
    );
    ...
    for (const scheduledTx of pendingTransactions) {
      ...
      try {
        if (scheduledTx.type === 'trade') {
          ...
          updateUserBalance(users, scheduledTx.buyerId!, -scheduledTx.amountZMW, scheduledTx.kWh);
          updateUserBalance(users, scheduledTx.sellerId!, scheduledTx.amountZMW, -scheduledTx.kWh);

          const newTransaction = createTransaction('trade', {
          ...
          updateUserBalance(users, scheduledTx.userId!, -scheduledTx.amountZMW, scheduledTx.kWh);
          
          const clusterIndex = clusters.findIndex(c => c.id === scheduledTx.clusterId);
          clusters[clusterIndex].availableKWh -= scheduledTx.kWh;

          const newTransaction = createTransaction('lease', {
